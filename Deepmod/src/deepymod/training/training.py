...
def train(
    ...
    **convergence_kwargs
    ...
    for iteration in torch.arange(0, max_iterations):
        ...
        if iteration % write_iterations == 0:
            ...
            while i < len(model.estimator_coeffs()[0]):
                ...
                coeffs_list[i] = np.append(coeffs_list[i], model.constraint_coeffs(sparse=True, scaled=False)[0][i].item())
                i=i+1
    ...
    filenum=6
    np.savetxt("iter_list%i.txt" % filenum, iter_list)
    np.savetxt("coeffs_list%i.txt" % filenum, coeffs_list)
    #################
