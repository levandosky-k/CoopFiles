...
class Logger:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #print("\n unscaled constraint coeffs\n")
        #print(unscaled_constraint_coeffs)
        #print("\n estimator coeffs\n")
        #print(estimator_coeffs)
    ...
    def update_tensorboard(
        ...
        **kwargs,
        ...
        for output_idx, (coeffs, unscaled_coeffs, estimator_coeffs) in enumerate(
            ...
            #print(coeffs.squeeze())
            #print(coeffs)
            #print(coeffs.item())
            #mycoeffs = torch.tensor([coeffs.item()])
            mycoeffs = torch.squeeze(coeffs, 0) #my addition
            #print(mycoeffs)
            self.writer.add_scalars(
                f"coeffs/output_{output_idx}",
                {f"coeff_{idx}": val for idx, val in enumerate(mycoeffs)}, #coeffs.squeeze())},
                iteration,
            )
            myunscaledcoeffs = torch.squeeze(unscaled_coeffs, 0) #my addition
            #print(myunscaledcoeffs)
            self.writer.add_scalars(
                f"unscaled_coeffs/output_{output_idx}",
                {
                    f"coeff_{idx}": val
                    for idx, val in enumerate(myunscaledcoeffs)  #unscaled_coeffs.squeeze())
                },
                iteration,
            )
            myestimatorcoeffs = np.squeeze(estimator_coeffs,0) #my addition
            #print(myestimatorcoeffs)
            self.writer.add_scalars(
                f"estimator_coeffs/output_{output_idx}",
                {
                    f"coeff_{idx}": val
                    for idx, val in enumerate(myestimatorcoeffs)  #estimator_coeffs.squeeze())
                },
                iteration,
            )
