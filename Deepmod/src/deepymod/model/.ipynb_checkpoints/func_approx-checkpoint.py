...
import matplotlib.pyplot as plt #my addition
...
class NN(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        #my additions########
        #print(self.network(coordinates).size()) #990 by 1
        #print(coordinates.size()) #990 by 2
        #print(self.network(coordinates))
        #print(coordinates)
        
        coords = coordinates.detach()
        data=self.network(coords).detach()
        filenum = 6
        if len(data) == 990:
            np.savetxt("learned_data_coords%i.txt" % filenum, coords)
            np.savetxt("learned_data_vals%i.txt" % filenum, data)
