...
def library_poly(prediction: torch.Tensor, max_order: int) -> torch.Tensor:
    ...
    return u
    #return torch.cat((u, torch.zeros_like(prediction)), dim=1)
    #return u[0:0] #returns empty list
...
def library_deriv(
    ...
    #return time_deriv, du[:, 2:max_order+1] 
...
class Library1D(Library):
    ...
    def library(
        ...
        filenum = 6
        np.savetxt("learned_ux_vals%i.txt" % filenum, torch.Tensor.detach(theta[:,1:2]))
        np.savetxt("learned_uxx_vals%i.txt" % filenum, torch.Tensor.detach(theta[:,2:3]))
        np.savetxt("learned_ut_vals%i.txt" % filenum, torch.Tensor.detach(time_deriv_list[0]))
        #print(len(theta[0][0]))
        #print(len(theta[:,1:2]))
        #theta = theta[:,2:3]
        #print([theta[:,2:3]]) #my addition
        #print([theta])
        #print([torch.cat((theta[:,0:1], theta[:,2:3]), 1)])
        #print(theta.size())
        #print(len(theta))
        #print([torch.cat((theta[:,3:4], theta[:,2:3]), 1)])
        return time_deriv_list, [theta] #[theta[:,2:3]] #[torch.cat((theta[:,3:4], theta[:,2:3]), 1)] #[theta]
