...
class Constraint(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, input: Tuple[TensorList, TensorList]) -> TensorList:
        ...
        #mysparsitymasks = [torch.tensor([self.sparsity_masks[0].item()])] #my addition
        self.coeff_vectors = [
    ...
    def apply_mask(thetas: TensorList, masks: TensorList) -> TensorList:
        ...
        """
        #print(masks)
        #mymasks = [torch.tensor([masks[0].item()])] #my addition
        #print(mymasks)
...
class Estimator(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, thetas: TensorList, time_derivs: TensorList) -> TensorList:
        ...
        with torch.no_grad():
            #print(torch.norm(time_derivs[0]))
            normed_time_derivs = [
                (time_deriv / torch.norm(time_deriv)).detach().cpu().numpy()
                for time_deriv in time_derivs
            ]
            #print(torch.norm(time_derivs[0]) / torch.norm(thetas[0], dim=0, keepdim=True))
            normed_thetas = [
                (theta / torch.norm(theta, dim=0, keepdim=True)).detach().cpu().numpy()
                for theta in thetas
            ]
            
            #print(torch.norm(thetas[0], dim=0, keepdim=True) / torch.norm(time_derivs[0]))
            
            #my additions###
            #normed_time_derivs = [
            #    (time_deriv).detach().cpu().numpy()
            #    for time_deriv in time_derivs
            #]
            #normed_thetas = [
...
class DeepMoD(nn.Module):
    ...
    def constraint_coeffs(self, scaled=False, sparse=False) -> TensorList:
        ...
        #my additions#######
        #print(coeff_vectors)
        #print(self.sparsity_masks[0])
        #print(self.sparsity_masks[0].item())
        #print(coeff_vectors[0].size())
        #print(self.library.norms)
        #print(self.library.norms[0])
        #print(self.library.norms[0].item())
        #mynorms = [torch.tensor([self.library.norms[0].item()])]
        #print(mynorms)
        #print(self.sparsity_masks)
        #mysparsitymasks = [torch.tensor([self.sparsity_masks[0].item()])] #my addition
        ####################
        if scaled:
            coeff_vectors = [
